...
class DataLoader3D(object):
    ...
    def generate_train_batch(self):
        ...
        for j, each_list in enumerate(selected_lists):
            ...
            if self.args.ted:
                ...
                assert ori_img.shape == ori_roi.shape == ori_artery.shape
                assert len(ori_img.shape) == len(ori_roi.shape) == len(ori_artery.shape) == 4
            ...
            if self.args.ted:
                data[j, 1] = np.pad(case_all_data[-1:], ((0, 0), 
                                                  (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                  (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                  (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                             "constant", **{'constant_values': -1})
    ...
    def prepare_data(self, args):
        ...
        ori_samples = [join(root_path, each_sample, each_sample+'_ace.npy') for each_sample in ori_samples]
        ori_samples.sort()
        ...
        for each_sample in ori_samples:
            ...
            if args.ted:
                path_list = [each_sample.replace('ace','img_ace'), each_sample, each_sample.replace('ace','roi')]
            else:
                path_list = [each_sample.replace('ace','img_ace'), each_sample]
