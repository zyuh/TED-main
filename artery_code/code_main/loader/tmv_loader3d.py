...
import random
import os
from augmentations.augment_spatial import augment_spatial, augment_spatialv2, get_patch_size
...
class DataLoader3D(object):
    ...
    def __init__(self, Basic_Args, split_flag):
        ...
        self.data_shape, self.seg_shape, self.density_shape = self.determine_shapes()
    ...
    def determine_shapes(self):
        if self.args.ted: # input=img+roi
            data_shape = (self.batch_size, 2, *self.basic_generator_patch_size)
            seg_shape = (self.batch_size, 1, *self.basic_generator_patch_size)
            density_shape = (self.batch_size, 1, *self.basic_generator_patch_size)
        else:
            data_shape = (self.batch_size, 1, *self.basic_generator_patch_size)
            seg_shape = (self.batch_size, 1, *self.basic_generator_patch_size)
            density_shape = (self.batch_size, 1, *self.basic_generator_patch_size)

        return data_shape, seg_shape, density_shape
    ...
    def generate_train_batch(self):
        ...
        density = np.zeros(self.density_shape, dtype=np.float32)
        ...
        for j, each_list in enumerate(selected_lists):
            ...
            # print(each_list)
            assert ori_img.shape == ori_artery.shape, print(ori_img.shape, ori_artery.shape)
            assert len(ori_img.shape) == len(ori_artery.shape) == 4, print(ori_img.shape, ori_artery.shape)

            if self.args.ted:
                ori_roi = np.load(each_list[2], 'r')[None]
                each_density_map = np.load(each_list[3], 'r')
                assert ori_img.shape == ori_roi.shape == ori_artery.shape == each_density_map.shape
                assert len(ori_img.shape) == len(ori_roi.shape) == len(ori_artery.shape) ==len(each_density_map.shape)== 4
                case_all_data = ori_img
                case_all_data = np.concatenate((case_all_data, ori_artery), axis=0)
                case_all_data = np.concatenate((case_all_data, ori_roi), axis=0)
                case_all_data = np.concatenate((case_all_data, each_density_map), axis=0)
            else:
                case_all_data = ori_img
                case_all_data = np.concatenate((case_all_data, ori_artery), axis=0)
            ...
            if self.args.ted:
                data[j, 1] = np.pad(case_all_data[2:3], ((0, 0), 
                                                  (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                  (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                  (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                             "constant", **{'constant_values': -1})

                density[j, 0] = np.pad(case_all_data[-1:], ((0, 0),
                                                        (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                        (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                        (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                                'constant', **self.pad_kwargs_data)
        ...
        if self.training:
            if self.args.ted:
                data, seg, density = augment_spatialv2(data, seg, density, self.patch_size, do_rotation=True, angle_x=self.rotation_x, angle_y=self.rotation_y, \
        ...
        if self.args.ted:
            data_dict = {'data': data, 'seg': seg, 'density': density}
            key_list = ['data', 'seg', 'density']
        else:
        ...
        data_dict['seg'] = seg
        if self.args.ted:
            roi = data_dict['data'][:, 1]
            roi[roi == -1] = 0
            data_dict['data'][:, 1] = roi
    ...
    def prepare_data(self, args):
        ...
        ori_samples = [join(root_path, each_sample, each_sample+'_tmv.npy') for each_sample in ori_samples]
        ori_samples.sort()
        ...
        for each_sample in ori_samples:
            ...
            if args.ted:
                path_list = [each_sample.replace('tmv','img_tmv'), each_sample, each_sample.replace('tmv','roi'), os.path.join(args.density_path, each_sample_id+'.npy')]
            else:
                path_list = [each_sample.replace('tmv','img_tmv'), each_sample]
