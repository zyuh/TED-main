...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x, mirror_axes, do_mirroring, mult) -> torch.tensor:
        ...
        for m in range(mirror_idx):
            if m == 0:
                pred = softmax_helper(self(x)) 
                result_torch += 1 / num_results * pred

            if m == 1 and (2 in mirror_axes):
                pred = softmax_helper(self(torch.flip(x, (4, ))))
                result_torch += 1 / num_results * torch.flip(pred, (4,))

            if m == 2 and (1 in mirror_axes):
                pred = softmax_helper(self(torch.flip(x, (3, ))))
                result_torch += 1 / num_results * torch.flip(pred, (3,))

            if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                pred = softmax_helper(self(torch.flip(x, (4, 3))))
                result_torch += 1 / num_results * torch.flip(pred, (4, 3))

            if m == 4 and (0 in mirror_axes):
                pred = softmax_helper(self(torch.flip(x, (2, ))))
                result_torch += 1 / num_results * torch.flip(pred, (2,))

            if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                pred = softmax_helper(self(torch.flip(x, (4, 2))))
                result_torch += 1 / num_results * torch.flip(pred, (4, 2))

            if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                pred = softmax_helper(self(torch.flip(x, (3, 2))))
                result_torch += 1 / num_results * torch.flip(pred, (3, 2))

            if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                pred = softmax_helper(self(torch.flip(x, (4, 3, 2))))
...
class ACE_UNet(SegmentationNetwork):
    def __init__(self, num_classes, act='leakyrelu', pool_op_kernel_sizes=None, conv_kernel_sizes=None, args=None): 
        ...
        self.seg_outputs = nn.Conv3d(self.conv_blocks_localization[len(self.conv_blocks_localization)-1][-1].output_channels, num_classes, 1, 1, 0, 1, 1, False)
