...
from loader.tmv_loader3d import DataLoader3D
...
def train_epoch(net, criterion, mse_fn, num_fn, optimizer, dataloader, amp_grad_scaler, Basic_Args=None):
    ...
    for i in range(Basic_Args.num_batches_per_epoch):
        ...
        if args.ted:
            densitys = data_dict['density']
            densitys = maybe_to_torch(densitys)
            if torch.cuda.is_available():
                densitys = to_cuda(densitys)    

        batch_size = data.size(0)

        optimizer.zero_grad()
        with autocast():
            outputs1, outputs2 = net(data)
            loss = criterion(outputs1, labels)

            if args.ted:
                outputs2_sigmoid = torch.sigmoid(outputs2)
                mse_loss = mse_fn(outputs2_sigmoid, densitys)
        ...
        if args.ted:
            progress_bar(i, Basic_Args.num_batches_per_epoch, 'train loss:{}, dcce loss:{}, mse loss:{}'.format(round(total_loss.item(), 4), round(loss.item(), 4), round(mse_loss.item(), 4)))
        else:
...
def evaluate(net, criterion, num_fn, dataset, Basic_Args=None):
    ...
    with torch.no_grad():
        for i in range(Basic_Args.num_val_batches_per_epoch):
            ...
            with autocast():
                ...
                if args.ted:
                    outputs2_sigmoid = torch.sigmoid(outputs2)
                    pred_num = torch.sum(outputs2_sigmoid)
                    gt_num = torch.sum(densitys)
                    num_loss = num_fn(pred_num, gt_num)

            total_loss += loss.item() * batch_size
            total += batch_size

            tmp_1, tmp_2, tmp_3, tmp_4 = run_online_evaluation(outputs1, labels)
    ...
    if args.ted:
        print('Val   loss: {:.5f}, Val   Dice: {:.5f}, num_loss:{:.5f}'.format(results['val_loss'], results['val_mean_dice'], num_loss))
    else:
...
def train(Basic_Args):
    ...
    print('Task: {}, method_name: {}'.format(Basic_Args.task_name, Basic_Args.method_name))
    ...
    ###=========================== model maker =======================###
    model = TMV_UNet(num_classes=2, act='leakyrelu', pool_op_kernel_sizes=None, conv_kernel_sizes=None, args=Basic_Args)
    model = model.cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method_name', type=str, default='ted')
