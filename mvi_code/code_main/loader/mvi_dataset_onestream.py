...
class MVI_Dataloader_mvi(object):
    def __init__(self, Basic_Args, phase, balance=False):
        ...
        self.lines, self.line_head = self.prepare_dataV4(split_flag=self.phase)
    ...
    def get_mean_std_for_each_attr(self):
        bio_marker_csv = './tednet/mvi_code/code_main/final_biomarkers.csv'
    ...
    def prepare_dataV4(self, split_flag='train'):

        root_path = self.args.data_path
        all_sample_list = os.listdir(root_path)
        all_sample_list = [os.path.join(root_path, each_sample) for each_sample in all_sample_list]
        all_sample_list.sort()
        sample_num = len(all_sample_list)
        train_num = int(self.args.split_prop * sample_num)
        # print(len(all_sample_list), all_sample_list[0], self.args.split_prop, sample_num)
        # print(all_sample_list[0])

        if split_flag=='train':
            train_sample_list = all_sample_list[:train_num] 
            examples_list = [join(each_sample, each_sample.split('/')[-1]+'_prepare_mvi.npy') for each_sample in train_sample_list]
        else:
            test_sample_list = all_sample_list[train_num:]
            examples_list = [join(each_sample, each_sample.split('/')[-1]+'_prepare_mvi.npy') for each_sample in test_sample_list]
            ...
            with open(bio_marker_csv, 'r') as f:
                ...
                for each_line in reader:
                    ...
                    if only_id == each_line[1]:
                        if self.mode == 'gt':
                            # bio_marker = [each_line[9], each_line[10], each_line[12], each_line[13], each_line[23], each_line[24], each_line[27], each_line[29]]
                            bio_marker = [each_line[2], each_line[9], each_line[10], each_line[12], each_line[13], each_line[23], each_line[24], each_line[27], each_line[29]]
                        elif self.mode == 'pred':
                            # bio_marker = [each_line[15], each_line[16], each_line[18], each_line[19], each_line[25], each_line[26], each_line[28], each_line[29]]
                            bio_marker = [each_line[2], each_line[15], each_line[16], each_line[18], each_line[19], each_line[25], each_line[26], each_line[28], each_line[29]]

                        if bio_marker[5]!='None': # 用来统计的
                            # artery_num_list.append(int(bio_marker[4]))
                            artery_num_list.append(int(bio_marker[5]))
                        mvi = each_line[2]
                        break
            f.close()
        ...
        self.artery_num_mean_num = np.mean(artery_num_list) 
        self.artery_num_std_num = np.std(artery_num_list)

        lists = positive_list + negative_list
        print('before balance, prepare {} data where positive {} and negative {}'.format(len(lists), len(positive_list), len(negative_list)))
        ...
        if self.balance: # only for train
            ...
            #     raise ValueError
            lists = positive_list + negative_list
            print('after balance, prepare {} data where positive {} and negative {} and scale {}'.format(len(lists), len(positive_list), len(negative_list), self.scale))

        if self.phase == 'train':
            print('for example, {}'.format(lists[0]))
    ...
    def generate_train_batch_V3(self):
        ...
        for j, line in enumerate(lines_plus):
            ...
            if self.phase == 'train':
                ...
                all_data_ori = copy.deepcopy(all_data_read[:, :, x:x+64, y:y+64, z:z+64])
            else:
            ...
            if self.use_attribute:
                ...
                if self.phase == 'train' and np.random.uniform() < 0.5:
                    ...
                    venous_data, artery_data = augment_gaussian_blur(venous_data, artery_data, p_per_channel=1) # only this work

                    ace_mask = np.flip(ace_mask, axis=axis)
                    fen_mask = np.flip(fen_mask, axis=axis)
                    cap_mask = np.flip(cap_mask, axis=axis)
                    artery_mask = np.flip(artery_mask, axis=axis)
                    ...
                    if self.do_rotation and np.random.uniform() < 0.5:
                        ...
                        ace_mask = np.rot90(ace_mask, k, rot_axis)
                        fen_mask = np.rot90(fen_mask, k, rot_axis)
                        cap_mask = np.rot90(cap_mask, k, rot_axis)
                        artery_mask = np.rot90(artery_mask, k, rot_axis)
