...
def show_npz(npz_path, save_path):
    ...
    cap_slice = npz_file['label_cap']
    fen_slice = npz_file['label_fen']
    roi_slice = npz_file['label_roi']
    fen_semantic = npz_file['label_fen_semantic']
    fen_degree = npz_file['label_fen_degree']
    cap_semantic = npz_file['label_cap_semantic']
    cap_degree = npz_file['label_cap_degree']

    maybe_mkdir_p(os.path.join(save_path, name))
    vis(fen_semantic, os.path.join(save_path, name, 'fen_semantic.png'), vis_center=False, center=None, center_gt=None)
    vis(cap_semantic, os.path.join(save_path, name, 'cap_semantic.png'), vis_center=False, center=None, center_gt=None)
    vis(fen_slice, os.path.join(save_path, name, 'fen_slice.png'), vis_center=False, center=None, center_gt=None)
    vis(cap_slice, os.path.join(save_path, name, 'cap_slice.png'), vis_center=False, center=None, center_gt=None)
    ...
    cv2.imwrite(os.path.join(save_path, name, 'fen_degree.png'), (fen_degree/np.max(fen_degree)*255).astype(np.uint8))
    cv2.imwrite(os.path.join(save_path, name, 'cap_degree.png'), (cap_degree/np.max(cap_degree)*255).astype(np.uint8))

# 单个样本的单个slice的cap/fen的转换,以cap命名
def label_convert_for_each_slice(cap_slice, roi_slice, name, args, label_type):
    print(name, label_type)
    label_cap = cap_slice
    label_hcc = roi_slice

    label_b, label_hcc_b, name_b = label_cap, label_hcc, name
    mask_gt = (label_b > 0).astype(np.float)
    label_recover_b, label_recover_degree_b = np.zeros_like(label_b), np.zeros_like(label_b)

    if np.sum(mask_gt)==0: 
        raise ValueError
    # 求质心
    M_gt = cv2.moments(mask_gt)
    cY_gt, cX_gt = int(M_gt["m10"] / M_gt["m00"]), int(M_gt["m01"] / M_gt["m00"])
    # 极坐标转换
    flatten_gt, r1, theta1 = cart2polar(mask_gt, M_gt, label_b, recover_flag=True, args=args) 
    # 消除歧义 每个角度上只能有一个label
    post_flatten_gt = post(flatten_gt, args=args) 
    # 使用插值平滑噪声
    post_flatten_gt = _smooth_flatten(post_flatten_gt, args=args) 
    # 补全 消除0 label （360度）这里还是序列
    post_flatten_gt, r1_comp, theta1_comp = _complete_flatten(post_flatten_gt.copy(), r1, theta1, args=args) 
    ...
    if np.sum(hcc_cvt_edge_b) == 0:  # data to be clean
        # do not extract by tumor, please just extract using cap
        hcc_cvt_edge_b = mask_gt
        M_cvt_gt = M_gt
        print("tumor label == 0 in case ", name_b)
    ...
    # 转换回直角坐标
    vertexs_b, seq_label_b, vertexs_offset_ctr_b = [], [], []
    # throw idx=0 away, the degree value range from 1 to 90!
    for idx_scale4 in range(90):
        ...
        for i in range(len(y_cart_new)):
            label_recover_b[np.int(x_cart_new[i])][np.int(y_cart_new[i])] = post_flatten_gt[idx, 0]
            label_recover_degree_b[np.int(x_cart_new[i])][np.int(y_cart_new[i])] = idx_scale4 + 1

        for i in range(len(y_cart_new_prev)):
            label_recover_b[np.int(x_cart_new_prev[i])][np.int(y_cart_new_prev[i])] = post_flatten_gt[idx, 0]
            label_recover_degree_b[np.int(x_cart_new_prev[i])][np.int(y_cart_new_prev[i])] = idx_scale4 + 1

        for i in range(len(y_cart_new_next)):
            label_recover_b[np.int(x_cart_new_next[i])][np.int(y_cart_new_next[i])] = post_flatten_gt[idx, 0] # 语义
            label_recover_degree_b[np.int(x_cart_new_next[i])][np.int(y_cart_new_next[i])] = idx_scale4 + 1 # 灰度级

    return label_recover_b, label_recover_degree_b


# 单个样本的转换
def main_label_convert(img_path, roi_path, cap_path, fen_path, each_uid_sample, save_path, args):
    ...
    for slice_th in range(len(img_npy)):
        ...
        cap_slice = cap_npy[slice_th]
        fen_slice = fen_npy[slice_th]
        # 跳过没有标注的slice
        if np.sum(cap_slice) == 0:
            continue
        if np.sum(fen_slice) == 0:
            continue

        mask_cap_gt = (cap_slice > 0).astype(np.float)
        M_gt = cv2.moments(mask_cap_gt)
        cY_gt, cX_gt = int(M_gt["m10"] / M_gt["m00"]), int(M_gt["m01"] / M_gt["m00"])

        # 排除边界标注错误填充的情况，改为用cap的标注得到边界
        if cap_slice[cX_gt, cY_gt] != 0:
            dilation = cv2.dilate(cap_slice.astype(np.uint8), kernel=np.ones((3, 3), np.uint8), iterations=1)
            erosion = cv2.erode(cap_slice.astype(np.uint8), kernel=np.ones((3, 3), np.uint8), iterations=1)
            mask_cap_gt_v2 = (dilation - erosion).astype(np.float)
            cap_slice = (cv2.GaussianBlur(mask_cap_gt_v2, (5, 5), 0) > 0).astype(np.float)

        # 因为标注是人工画的，所以cap,fen,roi可能对不准。。。
        mask_fen_gt = (fen_slice > 0).astype(np.float)
        M_gt = cv2.moments(mask_fen_gt)
        cY_gt, cX_gt = int(M_gt["m10"] / M_gt["m00"]), int(M_gt["m01"] / M_gt["m00"])  
        # 同理
        if fen_slice[cX_gt, cY_gt] != 0:
            dilation = cv2.dilate(fen_slice.astype(np.uint8), kernel=np.ones((3, 3), np.uint8), iterations=1)
            erosion = cv2.erode(fen_slice.astype(np.uint8), kernel=np.ones((3, 3), np.uint8), iterations=1)
            mask_fen_gt_v2 = (dilation - erosion).astype(np.float)
            fen_slice = (cv2.GaussianBlur(mask_fen_gt_v2, (5, 5), 0) > 0).astype(np.float)

        name = each_uid_sample+'_slice_'+str(slice_th)

        cap_semantic, cap_degree = label_convert_for_each_slice(cap_slice, roi_slice, name, args, label_type='cap')
        fen_semantic, fen_degree = label_convert_for_each_slice(fen_slice, roi_slice, name, args, label_type='fen')
        ...
        np.savez(save_path_final+'.npz', image=img_slice, label_cap=cap_slice, label_fen=fen_slice, label_roi=roi_slice,
                label_fen_semantic=fen_semantic, label_fen_degree=fen_degree, label_cap_semantic=cap_semantic, label_cap_degree=cap_degree)
...
def get_all_capfen_5mm_slice_multi_pool(args):
    ...
    for each_uid_sample in uid_sample_list:
        ...
        img_path = os.path.join(raw_data_root_path, each_uid_sample, each_uid_sample+'_img_5mm.nii')
        roi_path = os.path.join(raw_data_root_path, each_uid_sample, each_uid_sample+'_roi_5mm.nii')
        cap_path = os.path.join(raw_data_root_path, each_uid_sample, each_uid_sample+'_cap_5mm.nii')
        fen_path = os.path.join(raw_data_root_path, each_uid_sample, each_uid_sample+'_fen_5mm.nii')
        ...
        each_sample = [img_path, roi_path, cap_path, fen_path, each_uid_sample, args.save_path, args]
